        ...
        {items.map((item, idx) => {
          ...
          if (isOwnerView) {
            return (
              <li
                key={item.id}
                className={`${idx > 0 ? "border-t border-gray-100 dark:border-slate-700" : ""} ${
                  isClaimed ? "bg-emerald-50 dark:bg-emerald-900/20" : ""
                }`}
              >
                <div className="flex items-center justify-between gap-2 px-4 py-3.5">
                  <div className="min-w-0 flex-1">
                    <span className={`truncate text-sm ${isClaimed ? "text-gray-500 dark:text-slate-400" : "text-gray-700 dark:text-slate-200"}`}>
                      {item.quantity > 1 && (
                        <span className="mr-1 text-gray-400 dark:text-slate-500">{item.quantity}x</span>
                      )}
                      {item.name}
                    </span>
                    {claimant && (
                      <p className="text-xs text-emerald-600 font-medium mt-0.5 dark:text-emerald-400">
                        {claimant}
                      </p>
                    )}
                  </div>
                  <span className="flex shrink-0 items-center gap-2">
                      ...
                      <button
                        ...
                        className={`rounded-full px-2.5 py-1 text-[10px] font-semibold transition-all ${
                          item.isShared
                            ...
                            : "bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-600 dark:bg-slate-700 dark:text-slate-400 dark:hover:bg-slate-600 dark:hover:text-slate-300"
                        }`}
                    ...
                    <span className={`text-sm font-bold tabular-nums ${isClaimed ? "text-emerald-600 dark:text-emerald-400" : "text-gray-500 dark:text-slate-400"}`}>
              ...
              >
                ...
                <span className="flex flex-1 items-center justify-between gap-2 min-w-0">
                  <span className={`truncate text-sm transition-colors ${
                    checked ? "text-gray-900 font-medium dark:text-slate-100" : "text-gray-600 dark:text-slate-300"
                  }`}>
                    {item.quantity > 1 && (
                      <span className="mr-1 text-gray-400 dark:text-slate-500">{item.quantity}x</span>
                    )}
                  ...
                  <span className="flex shrink-0 items-center gap-2">
                    ...
                    <span className={`text-sm font-bold tabular-nums transition-colors ${
                      checked ? "text-teal-600 dark:text-teal-400" : "text-gray-500 dark:text-slate-400"
                    }`}>
                      {formatPrice(item.price, currency)}
                    </span>
                  </span>
